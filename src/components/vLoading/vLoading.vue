<template>
  <div class="v-loading-wrap" v-if="show" @touchmove.stop.prevent>
    <div class="v-loading-box" :class="[!!title?'title':'',theme]">
      <div class="v-loading" :class="'v-loading-' + type" :style="[cricleStyle]"></div>
 ...
 * @property {String} theme 主题色,仅有light/dark
 * @property {Boolean} show 是否显示动画
 * @property {String} title loading文字
 * @property {String} type 模式选择默认loading加载中(可选circle) success成功 warning警告
 * @property {String} color 动画活动区域的颜色，只对 type = flower 模式有效（默认#c7c7c7）
 * @property {String Number} size 加载图标的大小，单位px
 */
export default {
  name: "vLoading",
  data() {
